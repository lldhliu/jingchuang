...
class Swich(Base):
    ...
    swich = Column(SmallInteger, comment='开合状态')
    ...
    def get_data_list(cls, page, is_admin, uid, filter_by=None, filter_content=None):
        ...
        if is_admin:
            ...
            # r = Swich.query.join(Equipment, Equipment.id==Swich.equ_id).filter_by(**filter)\
            #     .paginate(page, per_page=10)
            return Swich.query.join(Equipment, Equipment.id==Swich.equ_id).filter_by(**filter)\
                .paginate(page, per_page=10)
    ...
    def get_last_data(cls, equ_id):
        return Swich.query.filter_by(equ_id=equ_id).order_by(desc(Swich.create_time)).first()
